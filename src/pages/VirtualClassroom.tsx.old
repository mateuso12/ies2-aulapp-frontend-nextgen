...
export const VirtualClassroom: React.FC = () => {
  ...
  const handleSelectOption = (exerciseId: string, optionId: string) => {
    ...
    // Verifica se é um tipo de exercício com opções
    if (
  ...
  const handleTryAgain = (exerciseId: string) => {
    ...
    // Limpa a resposta selecionada (para múltipla escolha) ou mantém (para writing)
    const exercise = mockExercises.find((ex) => ex.id === exerciseId)
  ...
  const handleVerifyWriting = (exerciseId: string) => {
    ...
    // Se correto, marca como submetido com comentário
    if (result.isCorrect) {
      const comment = exercise.metadata?.comment as
        | { title: string; content: string; imageUrl?: string }
        | undefined
              ...
              <div className="w-full md:w-[896px] mx-auto py-8">
                {(() => {
                  ...
                  if (isWritingExercise(exercise)) {
                    const currentAnswer =
                      (selectedAnswers[exercise.id] as string) || ''
                    const hasAnswer = currentAnswer.trim().length > 0
                    const submission = submittedAnswers[exercise.id]
                    const validationResult = validationResults[exercise.id]
                    const isSubmitted = !!submission && submission.isCorrect
                    ...
                    return (
                      ...
                      >
                        ...
                        <WritingExerciseComponent
                          ...
                          value={currentAnswer}
                          onChange={(answer) =>
                            handleWritingChange(exercise.id, answer)
                          }
                        ...
                        {!isSubmitted && !validationResult && (
                          <div className="flex justify-center">
                            <button
                              onClick={() => handleVerifyWriting(exercise.id)}
                              disabled={!hasAnswer}
                        ...
                        {submission &&
                          ...
                          resourceType === 'gamified' && (
                            ...
                            >
                              ...
                              {submission.comment.imageUrl && (
                                <img
                                  src={submission.comment.imageUrl}
                                  alt={submission.comment.title}
                  ...
                  return (
                    ...
                    >
                          ...
                          (option: MultipleChoiceOption, index: number) => {
                            ...
                            return (
                              <button
                                ...
                                onClick={() =>
                                  !isSubmitted &&
                                  handleSelectOption(exercise.id, option.id)
                                }
                      ...
                      {submittedAnswers[exercise.id] &&
                        !submittedAnswers[exercise.id].isCorrect && (
                      ...
                      {submittedAnswers[exercise.id]?.comment &&
                        submittedAnswers[exercise.id].isCorrect && (
                          <div className="flex justify-center pt-8">
                            <button
                              onClick={() => {
